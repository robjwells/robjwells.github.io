...
Catching up on my RSS feeds, I’ve just been through Dr Drang’s posts from nearly a year ago about switching away from TextExpander ([1][d1], [2][d2], [3][d3], [4][d4]).
...
But so far this is just like TextExpander’s own inline search. And because it’s part of LaunchBar you can’t actually use the snippets *in LaunchBar*. Why bother to switch?

Well, the overriding reason are still the non-technical: TextExpander 5 is done. Sooner or later it’s going to stop working. It’s not a matter of whether I should switch but what to.

LaunchBar is a good fit because I use it everywhere, all the time. The snippets are just text files on disk, and so can be deleted or renamed directly from LaunchBar like any other file. Snippets can be created directly from LaunchBar’s interface too by sending items to the Add Snippet action.
...
LaunchBar 6 was released in June 2014. Maybe the docs will be in LaunchBar 7?
