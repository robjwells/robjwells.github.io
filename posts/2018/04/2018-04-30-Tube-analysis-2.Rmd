...
For a start, let’s try to remake the first plot from [my previous post][tube-1], a plot of weekly spending with a six-week moving average. Looking back at that plot, it’s not tremendously helpful, but it’s a starting point.
...
With this summary frame assembled, we can now plot the totals.
...
ggplot(data = complete_week_totals,
    ...
    geom_point(
        ...
        size = 0.75) +
    geom_line(
        mapping = aes(y = mav(complete_week_totals$total, 6)),
        colour = rjw_red)
```
