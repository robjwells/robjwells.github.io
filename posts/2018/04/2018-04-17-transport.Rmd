...
new <- left_join(subsetted, top_pairs, by = 'station_pair')


station_cleaner <- function(x) {
    str_replace_all(x, ' DLR', '') %>%
...
ggplot(new) +
    ...
    labs(title = 'Most common journeys',
         subtitle = 'In either direction between stations, 10 or more trips',
...
```{r}
left_join(subsetted, top_pairs, by = 'station_pair')
```
