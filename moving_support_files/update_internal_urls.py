...
url_regex = re.compile(r'robjwells.com/post/(\d+)/?[-/\w]*')
base = 'robjwells.com'

def sub_id_for_url(match):
    """Rewrite a Tumblr URL to the new style"""
    try:
        new_path = post_map[match.group(1)]
        print(match.group(0), '\n', base + new_path, '\n')
...
for md_file in Path('.').glob('*/*.md'):
    ...
    rewritten = re.sub(url_regex, sub_id_for_url, text)
